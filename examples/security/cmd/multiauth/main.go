...
func main() {
	...
	case "localhost":
		...
		u = parseAddr(addr)
		svrs = append(svrs, newHTTPServer(u.Scheme, u.Host, securedServiceEndpoints, logger, *dbgF))
		if grpcAddr != "" {
			addr = grpcAddr
		} else {
		...
		u = parseAddr(addr)
		svrs = append(svrs, newGRPCServer(u.Scheme, u.Host, securedServiceEndpoints, logger, *dbgF))
	default:
